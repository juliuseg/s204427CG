...
window.onload = function init() {
    ...
    document.getElementById("kdSlider").oninput = function(event) {
        kd = vec3(event.target.value, 0.5, 0.31); // Adjust only the first component for simplicity
        ka = vec3(event.target.value, 0.5, 0.31); // Adjust only the first component for simplicity
    };  
